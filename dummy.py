...
def run(num_iter=100000, trial_size=100, 
    ...
    for ts in range(trial_size):
        ...
        for t in range(num_iter):
            ...
            if (t + 1) % 100 == 0:
                print('trial %3d: Iter %5d   MSE_yhat_y: %e MSE_gt: %e  SER: %e' % (ts+1, t + 1, mse_yhat_y, mse_gt, ser))
                # print('trial %3d: Iter %5d   MSE_yhat_y: %e MSE_gt: %e' % (ts+1, t + 1, mse_yhat_y, mse_gt))
        # Looking its best stopping point
        minvalue_nuh = min(record["mse_yhat_y"])
        ...
        print('record %0.2f trial %2d BSP(%d, %d) value(%e, %e) SER(%e, %e)' % (SNR_dB,ts+1, 
                                                                            bsp_nuh, bsp_gt,
        ...
        plt.plot(np.arange(num_iter), record["mse_yhat_y"])
