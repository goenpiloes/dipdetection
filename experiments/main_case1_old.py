...
def run(num_iter=100000, num_samples=100000,
    ...
    Htorch = torch.from_numpy(H).reshape(-1,1,2*Nr,2*Nt).type(dtype)
    # coef_y = torch.transpose(Htorch,2,3)
    
    b = torch.from_numpy(Y).type(dtype).reshape(-1,)
    # b = torch.matmul(coef_y,Y)
        
    z = torch.zeros_like(torch.empty(num_samples,1,Nt,2)).type(dtype).normal_()
...
for SNR_dB in arrSNR_dB:
    ...
    run(num_iter=2000,
        ...
        savedataset=False, writer=writerdata, ser = ser, ser1 = ser1,
        seed=None)
toc = time.time()
