...
def run(num_iter=100000, num_samples=100000,
    ...
    for ts in range(num_samples):
        Ytorch = torch.from_numpy(Y[ts]).type(dtype).reshape(1,1,-1,1)
        Htorch = torch.from_numpy(H[ts]).reshape(1,1,2*Nr,2*Nt).type(dtype)
        
        # do the MRC
        Ht_dot_Y = torch.matmul(torch.transpose(Htorch,2,3), Ytorch)
        diag_Ht_dot_H = torch.diagonal(torch.matmul(torch.transpose(Htorch,2,3),Htorch),offset=0, dim1=2, dim2=3).reshape(1,1,-1,1)
        
        b = (Ht_dot_Y/diag_Ht_dot_H).reshape(-1,)
        
        G.apply(weight_reset)
        z = torch.zeros_like(torch.empty(1,16)).type(dtype).normal_()
...
datasets_dir = '../data/test3_copy/'
if not os.path.isdir(datasets_dir):
    os.makedirs(datasets_dir)
results_dir = '../data/test3_copy/results/'
