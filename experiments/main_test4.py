...
def run(num_iter=100000, num_samples=100000,
    ...
    # Build autoencoder architecture neural network
    G = nn.Sequential(
        nn.Linear(2,8),
        nn.LeakyReLU(),
        nn.Linear(8,8),
    ...
    for ts in range(num_samples):
        ...
        z = torch.zeros_like(torch.empty(1,2)).type(dtype).normal_()
...
datasets_dir = '../data/test4/'
if not os.path.isdir(datasets_dir):
    os.makedirs(datasets_dir)
results_dir = '../data/test4/results4/'
