# Run several cases at the same time --> N > 1 in input size = (N,C,H,W)
...
def run(num_iter=100000, num_samples=100000, num_epoch_like = 1000,
    ...
    X = X.reshape(num_epoch_like,-1,1,2*Nt,1)
    Y = Y.reshape(num_epoch_like,-1,1,2*Nr,1)
    H = H.reshape(num_epoch_like,-1,1,2*Nr,2*Nt)
    ...
    mat_xhat = np.array([])
    mat_xtrue = np.array([])
    for epoch_like in range(num_epoch_like):
        b = torch.from_numpy(Y[epoch_like]).type(dtype).reshape(-1,)
        
        Htorch = torch.from_numpy(H[epoch_like]).reshape(-1,1,2*Nr,2*Nt).type(dtype)
        
        G.apply(weight_reset)
        
        z = torch.zeros_like(torch.empty(Htorch.shape[0],1,Nt,2)).type(dtype).normal_()
        ...
        for t in range(num_iter):
            ...
            X_hat = (torch.transpose(X_hat,2,3).reshape(-1,1,Nt*2,1))*2-1
            ...
            X_hat_numpy = X_hat.detach().cpu().numpy().reshape((-1,1,Nt*2,1))
            
            mse_gt = np.mean((X_hat_numpy - X[epoch_like]) ** 2)
            ser_instant = CalcSER(X[epoch_like], X_hat_numpy, M)
            ...
            if (t + 1) % 100 == 0:
                print('epoch_like %5d: Iter %5d  fidelity_loss: %e  MSE_xhat_x: %e' % (epoch_like+1, t + 1, fidelity_loss.item(), mse_gt))
        ...
        print('record %0.2f epoch_like %2d value: %e' % (SNR_dB,epoch_like+1,minvalue_gt))
   
        mat_xhat = np.append(mat_xhat, X_hat_numpy.reshape(-1,1))
        mat_xtrue = np.append(mat_xtrue, X[epoch_like].reshape(-1,1))
...
datasets_dir = '../data/test2/'
if not os.path.isdir(datasets_dir):
    os.makedirs(datasets_dir)
results_dir = '../data/test2/results/'
