...
def run(num_iter=100000, num_samples=100000,
    ...
    for ts in range(num_samples):
        ...
        z = torch.zeros_like(torch.empty(1,16,4,4)).type(dtype).normal_() # number of channel (dim=1) must be same with number of channel input in the neural network
...
datasets_dir = '../data/test5_v2/'
if not os.path.isdir(datasets_dir):
    os.makedirs(datasets_dir)
results_dir = '../data/test5_v2/results1/'
